...
## Run 92 (Cycle 92 of 300 campaign)

- Date: 2026-03-02
- Cloud track: Azure (round-robin assignment)
- Objective: add Delta Sharing relationship-list coverage for provider and recipient share views.

### Progress Notes

- Added relationship-list wrappers in `src/databricks_api/clients/workspace.py`:
  - `list_sharing_provider_shares`
  - `list_share_recipient_shares`
- Updated `src/databricks_api/endpoints/catalog.py` with provider/recipient share-list endpoint keys.
...
## Run 96 (Cycle 96 of 300 campaign)

- Date: 2026-03-02
- Cloud track: GCP (round-robin assignment)
- Objective: extend Delta Sharing relationship coverage with provider/recipient share-link create/delete operations.

### Progress Notes

- Added share-link lifecycle wrappers in `src/databricks_api/clients/workspace.py`:
  - `create_sharing_provider_share`, `delete_sharing_provider_share`
  - `create_share_recipient_share`, `delete_share_recipient_share`
- Updated `src/databricks_api/endpoints/catalog.py` with provider/recipient share create/delete endpoint keys.
- Added regression coverage in `tests/test_workspace_client.py` for routing and validation paths.
...
## Run 98 (Cycle 98 of 300 campaign)

- Date: 2026-03-02
- Cloud track: Azure (round-robin assignment)
- Objective: expand Delta Sharing from the share-centric side with provider/recipient relationship lookups.

### Progress Notes

- Added share-centric wrappers in `src/databricks_api/clients/workspace.py`:
  - `list_share_providers`, `get_share_provider`
  - `list_share_recipients_for_share`, `get_share_recipient_for_share`
- Updated `src/databricks_api/endpoints/catalog.py` with share providers/recipients list/detail endpoint keys.
- Added regression coverage in `tests/test_workspace_client.py` for routing and validation paths.
...
## Run 104 (Cycle 104 of 300 campaign)

- Date: 2026-03-02
- Cloud track: Azure (round-robin assignment)
- Objective: harden Delta Sharing core share payload validation for create/update operations.

### Progress Notes

- Strengthened payload validation in `src/databricks_api/clients/workspace.py` for:
  - `create_share` (`share_spec` must be non-empty)
  - `update_share` (`share_changes` must be non-empty)
- Added regression coverage in `tests/test_workspace_client.py` for empty payload rejection in share create/update flows.
- Updated `docs/COMPLETE_TOOL_USAGE_REFERENCE.md` with explicit share payload requirements.
- Prepared patch version bump to `2.0.27` with changelog updates for release.

## Run 105 (Cycle 105 of 300 campaign)

- Date: 2026-03-02
- Cloud track: GCP (round-robin assignment)
- Objective: harden Delta Sharing provider/recipient core payload validation for create/update operations.

### Progress Notes

- Strengthened payload validation in `src/databricks_api/clients/workspace.py` for:
  - `create_sharing_provider` and `update_sharing_provider`
  - `create_share_recipient` and `update_share_recipient`
...
## Run 107 (Cycle 107 of 300 campaign)

- Date: 2026-03-02
- Cloud track: Azure (round-robin assignment)
- Objective: harden Marketplace provider/file payload validation for create/update operations.

### Progress Notes

- Strengthened payload validation in `src/databricks_api/clients/workspace.py` for:
  - `create_marketplace_provider_provider` and `update_marketplace_provider_provider`
  - `create_marketplace_provider_file` and `update_marketplace_provider_file`
...
## Run 109 (Cycle 109 of 300 campaign)

- Date: 2026-03-02
- Cloud track: AWS (round-robin assignment)
- Objective: harden Marketplace personalization and analytics payload validation for create/update operations.

### Progress Notes

- Strengthened payload validation in `src/databricks_api/clients/workspace.py` for:
  - `update_marketplace_provider_personalization_request`
  - `create_marketplace_consumer_personalization_request`
  - `update_marketplace_consumer_personalization_request`
...
## Run 111 (Cycle 111 of 300 campaign)

- Date: 2026-03-02
- Cloud track: GCP (round-robin assignment)
- Objective: harden Global Init Script payload validation for create/update operations.
...
- Added regression coverage in `tests/test_workspace_client.py` for empty payload rejection in global init script create/update flows.
- Updated `docs/COMPLETE_TOOL_USAGE_REFERENCE.md` with explicit global init script payload requirements.
- Prepared patch version bump to `2.0.34` with changelog updates for release.
