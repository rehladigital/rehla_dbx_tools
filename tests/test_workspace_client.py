...
def test_marketplace_and_model_serving_wrappers_route_expected_calls_and_validation():
    ...
    with pytest.raises(ValidationError):
        client.create_marketplace_provider_provider({})
    with pytest.raises(ValidationError):
        client.update_marketplace_provider_provider("", {})
    with pytest.raises(ValidationError):
        client.update_marketplace_provider_provider("pp-1", {})
    with pytest.raises(ValidationError):
        client.delete_marketplace_provider_provider("")
    with pytest.raises(ValidationError):
        client.get_marketplace_provider_file("")
    with pytest.raises(ValidationError):
        client.create_marketplace_provider_file({})
    with pytest.raises(ValidationError):
        client.update_marketplace_provider_file("", {})
    with pytest.raises(ValidationError):
        client.update_marketplace_provider_file("pf-1", {})
    with pytest.raises(ValidationError):
...
def test_files_and_sharing_wrappers_route_expected_calls_and_validation():
    ...
    with patch.object(client, "request_versioned", return_value="ok") as request_versioned:
        ...
        client.create_sharing_provider_share("partner-a", {"share_name": "sales_share"})
        assert request_versioned.call_args.args == ("POST", "unity-catalog")
        assert request_versioned.call_args.kwargs["endpoint"] == "providers/partner-a/shares"
        assert request_versioned.call_args.kwargs["json_body"] == {"share_name": "sales_share"}

        client.delete_sharing_provider_share("partner-a", "sales_share")
        assert request_versioned.call_args.args == ("DELETE", "unity-catalog")
        assert request_versioned.call_args.kwargs["endpoint"] == "providers/partner-a/shares/sales_share"

        client.update_sharing_provider_share("partner-a", "sales_share", {"comment": "linked"})
        assert request_versioned.call_args.args == ("PATCH", "unity-catalog")
        assert request_versioned.call_args.kwargs["endpoint"] == "providers/partner-a/shares/sales_share"
        assert request_versioned.call_args.kwargs["json_body"] == {"comment": "linked"}
        ...
        client.create_share_recipient_share("consumer-a", {"share_name": "sales_share"})
        assert request_versioned.call_args.args == ("POST", "unity-catalog")
        assert request_versioned.call_args.kwargs["endpoint"] == "recipients/consumer-a/shares"
        assert request_versioned.call_args.kwargs["json_body"] == {"share_name": "sales_share"}

        client.delete_share_recipient_share("consumer-a", "sales_share")
        assert request_versioned.call_args.args == ("DELETE", "unity-catalog")
        assert request_versioned.call_args.kwargs["endpoint"] == "recipients/consumer-a/shares/sales_share"

        client.update_share_recipient_share("consumer-a", "sales_share", {"comment": "recipient-linked"})
        assert request_versioned.call_args.args == ("PATCH", "unity-catalog")
        assert request_versioned.call_args.kwargs["endpoint"] == "recipients/consumer-a/shares/sales_share"
        assert request_versioned.call_args.kwargs["json_body"] == {"comment": "recipient-linked"}
    ...
    with pytest.raises(ValidationError):
        client.create_share_provider_link("sales_share", {})
    with pytest.raises(ValidationError):
        client.update_share_provider_link("", "partner-a", {})
    with pytest.raises(ValidationError):
        client.update_share_provider_link("sales_share", "", {})
    with pytest.raises(ValidationError):
        client.update_share_provider_link("sales_share", "partner-a", {})
    with pytest.raises(ValidationError):
    ...
    with pytest.raises(ValidationError):
        client.create_share_recipient_link("sales_share", {})
    with pytest.raises(ValidationError):
        client.update_share_recipient_link("", "consumer-a", {})
    with pytest.raises(ValidationError):
        client.update_share_recipient_link("sales_share", "", {})
    with pytest.raises(ValidationError):
        client.update_share_recipient_link("sales_share", "consumer-a", {})
    with pytest.raises(ValidationError):
